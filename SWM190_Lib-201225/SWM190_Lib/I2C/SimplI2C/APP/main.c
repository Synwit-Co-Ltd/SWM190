...
void I2CMstInit(void)
{
	...
	PORT_Init(PORTD, PIN5, PORTD_PIN5_I2C0_SCL, 1);	//GPIOD.5配置为I2C0 SCL引脚
	PORTD->PULLU |= (1 << PIN5);					//必须使能上拉，用于模拟开漏
	PORT_Init(PORTD, PIN4, PORTD_PIN4_I2C0_SDA, 1);	//GPIOD.4配置为I2C0 SDA引脚
	PORTD->PULLU |= (1 << PIN4);					//必须使能上拉，用于模拟开漏
...
void I2CSlvInit(void)
{
	...
	PORT_Init(PORTA, PIN15, PORTA_PIN15_I2C1_SCL, 1);	//GPIOA.15配置为I2C1 SCL引脚
	PORTA->PULLU |= (1 << PIN15);						//必须使能上拉，用于模拟开漏
	PORT_Init(PORTA, PIN14, PORTA_PIN14_I2C1_SDA, 1);	//GPIOA.14配置为I2C1 SDA引脚
	PORTA->PULLU |= (1 << PIN14);						//必须使能上拉，用于模拟开漏
